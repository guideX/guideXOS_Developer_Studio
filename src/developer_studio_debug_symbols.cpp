...
static uint32_t shaRotateRight(uint32_t value, uint32_t count) {
...
static void sha256Transform(Sha256State* state, const unsigned char* block) {
    ...
    for (uint32_t i = 16; i < 64; ++i) {
        const uint32_t first = words[i - 15];
        const uint32_t second = words[i - 2];
        const uint32_t small0 = shaRotateRight(first, 7) ^ shaRotateRight(first, 18) ^ (first >> 3);
        const uint32_t small1 = shaRotateRight(second, 17) ^ shaRotateRight(second, 19) ^ (second >> 10);
    ...
    for (uint32_t i = 0; i < 64; ++i) {
        const uint32_t big1 = shaRotateRight(e, 6) ^ shaRotateRight(e, 11) ^ shaRotateRight(e, 25);
        const uint32_t choose = (e & f) ^ ((~e) & g);
        const uint32_t temp1 = h + big1 + choose + constants[i] + words[i];
        const uint32_t big0 = shaRotateRight(a, 2) ^ shaRotateRight(a, 13) ^ shaRotateRight(a, 22);
...
static void sha256Update(Sha256State* state, const unsigned char* bytes, uint64_t size) {
    if (!state || (!bytes && size != 0)) return;
    state->byteCount += size;
    while (size != 0) {
        const uint32_t available = 64u - state->buffered;
        const uint32_t take = size < available ? static_cast<uint32_t>(size) : available;
        for (uint32_t i = 0; i < take; ++i) state->block[state->buffered + i] = bytes[i];
        state->buffered += take;
        bytes += take;
        size -= take;
...
static void sha256Final(Sha256State* state, unsigned char digest[32]) {
    const uint64_t bitCount = state->byteCount * 8u;
    state->block[state->buffered++] = 0x80u;
    if (state->buffered > 56u) {
        while (state->buffered < 64u) state->block[state->buffered++] = 0;
        sha256Transform(state, state->block);
        state->buffered = 0;
    }
    while (state->buffered < 56u) state->block[state->buffered++] = 0;
    for (uint32_t i = 0; i < 8; ++i) state->block[56u + i] = static_cast<unsigned char>(bitCount >> (56u - i * 8u));
    sha256Transform(state, state->block);
    for (uint32_t i = 0; i < 8; ++i) {
        digest[i * 4] = static_cast<unsigned char>(state->hash[i] >> 24);
        digest[i * 4 + 1] = static_cast<unsigned char>(state->hash[i] >> 16);
        digest[i * 4 + 2] = static_cast<unsigned char>(state->hash[i] >> 8);
        digest[i * 4 + 3] = static_cast<unsigned char>(state->hash[i]);
    }
}
...
bool DebugDwarfComputeSha256(const unsigned char* bytes, uint64_t size,
    ...
    if (!bytes || size == 0 || !output || outputSize < 65u) {
        if (output && outputSize != 0) output[0] = '\0';
        return false;
    }
    Sha256State state;
    sha256Init(&state);
    sha256Update(&state, bytes, size);
    unsigned char digest[32] = {};
    sha256Final(&state, digest);
    for (uint32_t i = 0; i < 32; ++i) {
        output[i * 2] = digits[digest[i] >> 4];
        output[i * 2 + 1] = digits[digest[i] & 0xfu];
    }
    output[64] = '\0';
    return true;
}
