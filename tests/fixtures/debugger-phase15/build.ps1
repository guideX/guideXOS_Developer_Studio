...
$flags = @('--target=x86_64-unknown-elf','-std=c++11','-O0','-g','-ffreestanding','-fno-exceptions','-fno-rtti','-fno-stack-protector','-fno-omit-frame-pointer','-fno-unwind-tables','-fno-asynchronous-unwind-tables',"-I$SdkInclude")
if ($Configuration -eq "Debug") { $flags = $flags | Where-Object { $_ -ne '-g' } }
...
try {
    ...
    $elfPath = Join-Path $PackageBin "debugger-phase15.elf"
