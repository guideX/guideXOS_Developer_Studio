...
extern "C" __attribute__((noinline)) int level3(int value) {
    g_result = value + 1; // Step Out proof breakpoint
    return g_result;
}
...
extern "C" gx_result GX_CALL gx_main(gx_app_context* ctx) {
    ...
    gx_result result = ctx->host->request_window_ex ?
        ctx->host->request_window_ex(ctx, "Debugger Phase 8 Fixture", 640, 360,
    ...
    if (ctx->host->draw_text)
        ctx->host->draw_text(ctx, g_window, 24, 40, "Debugger Phase 8 Fixture");
    if (ctx->host->log)
        ctx->host->log(ctx, resultValue == 4 ? "step-out fixture ready" : "step-out fixture failed");
