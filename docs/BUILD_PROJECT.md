...
The request, polling, and release calls are appended to the Native Host Call Table. The Server runs the process on a worker thread, captures merged stdout/stderr without blocking the compositor, and returns bounded snapshots. Only the owning runtime can release a completed handle. A second build is rejected while one is active. Closing the project, workspace, or application is blocked while a build is active; there is no implicit cancellation command in this slice.
