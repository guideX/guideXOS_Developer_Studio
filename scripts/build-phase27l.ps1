...
$PackageRoot = Join-Path $ServerRoot "Apps\DS27L"
$PackageBin = Join-Path $PackageRoot "bin\amd64"
$ObjectRoot = Join-Path $ServerRoot "tmp\developer-studio-phase27l-build"
$ElfPath = Join-Path $PackageBin "p27l.elf"
$StagedElf = Join-Path $ObjectRoot "p27l.elf"
...
function Normalize-NativeElf([string]$Path) {
    ...
    if ($bytes.Length -lt 64 -or $bytes[0] -ne 0x7f -or $bytes[1] -ne 0x45 -or $bytes[2] -ne 0x4c -or $bytes[3] -ne 0x46) { throw "Phase 27L ELF header is invalid." }
    $phoff = [BitConverter]::ToInt64($bytes, 32)
    $phentsize = [BitConverter]::ToUInt16($bytes, 54)
    $phnum = [BitConverter]::ToUInt16($bytes, 56)
    if ($phoff -ne 64 -or $phentsize -ne 56) { throw "Unexpected Phase 27L program-header layout." }
    ...
    if ($loads.Count -eq 0 -or $loads.Count -gt 4) { throw "Phase 27L ELF has an unsupported PT_LOAD count." }
...
if (-not $clang -or -not $lld) { throw "clang++ and ld.lld are required for the Phase 27L app proof." }
...
Copy-Item -LiteralPath (Join-Path $RepoRoot "app\phase27l_app.json") -Destination (Join-Path $PackageRoot "app.json") -Force
...
$compileFlags = @(
    ...
    "-DGXOS_PHASE27L_APP", "-I$SdkInclude", "-Isrc"
)
...
if (-not (Test-Path -LiteralPath $StagedElf -PathType Leaf) -or (Get-Item -LiteralPath $StagedElf).Length -le 0) { throw "Phase 27L NativeElf output was not produced." }
Move-Item -LiteralPath $StagedElf -Destination $ElfPath -Force
Write-Host "Developer Studio Phase 27L NativeElf proof app built: $ElfPath" -ForegroundColor Green
