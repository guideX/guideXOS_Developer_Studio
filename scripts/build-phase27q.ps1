...
$PackageRoot = Join-Path $ServerRoot "Apps\DS27Q"
$PackageBin = Join-Path $PackageRoot "bin\amd64"
$ObjectRoot = Join-Path $ServerRoot "tmp\developer-studio-phase27q-build"
$ElfPath = Join-Path $PackageBin "p27q.elf"
$StagedElf = Join-Path $ObjectRoot "p27q.elf"
...
function Normalize-NativeElf([string]$Path) {
    ...
    if ($bytes.Length -lt 64 -or $bytes[0] -ne 0x7f -or $bytes[1] -ne 0x45 -or $bytes[2] -ne 0x4c -or $bytes[3] -ne 0x46) { throw "Phase 27Q ELF header is invalid." }
    $phoff = [BitConverter]::ToInt64($bytes, 32)
    $phentsize = [BitConverter]::ToUInt16($bytes, 54)
    $phnum = [BitConverter]::ToUInt16($bytes, 56)
    if ($phoff -ne 64 -or $phentsize -ne 56) { throw "Unexpected Phase 27Q program-header layout." }
    ...
    if ($loads.Count -eq 0 -or $loads.Count -gt 4) { throw "Phase 27Q ELF has an unsupported PT_LOAD count." }
...
if (-not $clang -or -not $lld) { throw "clang++ and ld.lld are required for the Phase 27Q proof app bootstrap." }
...
Copy-Item -LiteralPath (Join-Path $RepoRoot "app\phase27q_app.json") -Destination (Join-Path $PackageRoot "app.json") -Force
...
$compileFlags = @(
    ...
    "-DGXOS_PHASE27Q_APP", "-I$SdkInclude", "-Isrc"
)
...
if (-not (Test-Path -LiteralPath $StagedElf -PathType Leaf) -or (Get-Item -LiteralPath $StagedElf).Length -le 0) { throw "Phase 27Q NativeElf output was not produced." }
Move-Item -LiteralPath $StagedElf -Destination $ElfPath -Force
Write-Host "Developer Studio Phase 27Q NativeElf proof app built: $ElfPath" -ForegroundColor Green
