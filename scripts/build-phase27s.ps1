...
$PackageRoot = Join-Path $ServerRoot "Apps\DS27S"
$PackageBin = Join-Path $PackageRoot "bin\amd64"
$ObjectRoot = Join-Path $ServerRoot "tmp\developer-studio-phase27s-build"
$ElfPath = Join-Path $PackageBin "p27s.elf"
$StagedElf = Join-Path $ObjectRoot "p27s.elf"
...
function Normalize-NativeElf([string]$Path) {
    ...
    if ($bytes.Length -lt 64 -or $bytes[0] -ne 0x7f -or $bytes[1] -ne 0x45 -or $bytes[2] -ne 0x4c -or $bytes[3] -ne 0x46) { throw "Phase 27S ELF header is invalid." }
    $phoff = [BitConverter]::ToInt64($bytes, 32)
    $phentsize = [BitConverter]::ToUInt16($bytes, 54)
    $phnum = [BitConverter]::ToUInt16($bytes, 56)
    if ($phoff -ne 64 -or $phentsize -ne 56) { throw "Unexpected Phase 27S program-header layout." }
    $loads = New-Object System.Collections.Generic.List[byte[]]
    for ($i = 0; $i -lt $phnum; ++$i) {
        $offset = [int]($phoff + ($i * $phentsize))
        if (($bytes[$offset] -bor ($bytes[$offset + 1] -shl 8) -bor ($bytes[$offset + 2] -shl 16) -bor ($bytes[$offset + 3] -shl 24)) -eq 1) {
            $header = New-Object byte[] $phentsize
            [Array]::Copy($bytes, $offset, $header, 0, $phentsize)
            $loads.Add($header)
        }
    }
    if ($loads.Count -eq 0 -or $loads.Count -gt 4) { throw "Phase 27S ELF has an unsupported PT_LOAD count." }
    for ($i = 0; $i -lt $loads.Count; ++$i) { [Array]::Copy($loads[$i], 0, $bytes, [int]($phoff + ($i * $phentsize)), $phentsize) }
    $bytes[56] = [byte]($loads.Count -band 0xff); $bytes[57] = [byte](($loads.Count -shr 8) -band 0xff)
...
if (-not $clang -or -not $lld) { throw "clang++ and ld.lld are required for the Phase 27S proof app bootstrap." }
...
Copy-Item -LiteralPath (Join-Path $RepoRoot "app\phase27s_app.json") -Destination (Join-Path $PackageRoot "app.json") -Force
if (Test-Path -LiteralPath $ElfPath) { Remove-Item -LiteralPath $ElfPath -Force }
...
$compileFlags = @(
    ...
    "-DGXOS_PHASE27S_APP", "-I$SdkInclude", "-Isrc"
)
...
if (-not (Test-Path -LiteralPath $StagedElf) -or (Get-Item -LiteralPath $StagedElf).Length -le 0) { throw "Phase 27S NativeElf output was not produced." }
Move-Item -LiteralPath $StagedElf -Destination $ElfPath -Force
Write-Host "Developer Studio Phase 27S NativeElf proof app built: $ElfPath" -ForegroundColor Green
